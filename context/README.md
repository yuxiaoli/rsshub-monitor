...
```bash
uv run python src/rsshub_monitor/scan_servers.py [options]
```
...
```bash
uv run python src/rsshub_monitor/scan_namespace.py [options]
```
...
```bash
uv run python src/rsshub_monitor/split_by_category.py [input_file] [options]
```
...
uv run python src/rsshub_monitor/split_by_category.py

# Create summary files with default 100 items limit
uv run python src/rsshub_monitor/split_by_category.py -s

# Create summary files with custom items limit
uv run python src/rsshub_monitor/split_by_category.py -s 50
```
